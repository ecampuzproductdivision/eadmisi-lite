  ...
  <div class="card border-1 mb-6">
    <div class="card-body py-4">
      <div class="row align-items-center">
        <div class="col">
          <h3 class="mb-1 fw-bold">Tambah Dosen</h3>
        ...
        <div class="col-auto">
          <a href="{{ route('dosen.index') }}" class="btn btn-light border fw-semibold text-dark"><i class="ti ti-arrow-left me-1"></i> Kembali</a>
        </div>
      </div>
    </div>
  </div>
  ...
  <form action="{{ route('dosen.store') }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
    ...
    <div class="row g-4">
      ...
      <div class="col-12 col-xl-6">
        <div class="card card-lg h-100 mb-6">
          ...
          <div class="card-body">
            <div class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <input type="text" name="nidn" class="form-control" value="{{ old('nidn') }}" maxlength="10">
              </div>
              <div class="col-md-6">
                <label class="form-label">NIDK</label>
                <input type="text" name="nidk" class="form-control" value="{{ old('nidk') }}" maxlength="10">
              </div>
              <div class="col-md-6">
                <label class="form-label">NUPN</label>
                <input type="text" name="nupn" class="form-control" value="{{ old('nupn') }}" maxlength="10">
              </div>
              <div class="col-12">
              ...
              <div class="col-md-6">
                ...
                <input type="text" name="gelar_depan" class="form-control" value="{{ old('gelar_depan') }}" placeholder="Contoh: Dr. Ir.">
              </div>
              <div class="col-md-6">
                <label class="form-label">Gelar Belakang</label>
                <input type="text" name="gelar_belakang" class="form-control" value="{{ old('gelar_belakang') }}" placeholder="Contoh: M.Kom.">
              </div>
          ...
          <div class="card-body">
            <div class="row g-3">
              <div class="col-12">
                ...
                <select name="id_prodi" class="form-select" required>
                  ...
                  @foreach($prodiList as $p)
                    <option value="{{ $p->prodiKode }}" {{ old('id_prodi') == $p->prodiKode ? 'selected' : '' }}>
              ...
              <div class="col-md-6">
                ...
                <select name="jenis_dosen" class="form-select" required>
                  @foreach(['Tetap', 'Tidak Tetap', 'Luar Biasa', 'Tamu'] as $jd)
                    <option value="{{ $jd }}" {{ old('jenis_dosen') == $jd ? 'selected' : '' }}>{{ $jd }}</option>
                  @endforeach
                </select>
              </div>
              <div class="col-md-6">
              ...
              <div class="col-md-6">
                ...
                <select name="status_dosen" class="form-select" required>
                  @foreach(['Aktif', 'Cuti', 'Tugas Belajar', 'Pensiun', 'Wafat', 'Nonaktif'] as $sd)
                    <option value="{{ $sd }}" {{ old('status_dosen') == $sd ? 'selected' : '' }}>{{ $sd }}</option>
                  @endforeach
                </select>
              </div>
              <div class="col-md-6">
                <label class="form-label">Mulai Bertugas <span class="text-danger">*</span></label>
                <input type="date" name="tanggal_mulai_bertugas" class="form-control" value="{{ old('tanggal_mulai_bertugas') }}" required>
              </div>
              <div class="col-md-4">
                ...
                <select name="jenjang_pendidikan_terakhir" class="form-select" required>
                  ...
                  @foreach(['S1', 'S2', 'S3'] as $jenjang)
                    <option value="{{ $jenjang }}" {{ old('jenjang_pendidikan_terakhir') == $jenjang ? 'selected' : '' }}>{{ $jenjang }}</option>
                  @endforeach
                </select>
              </div>
              <div class="col-md-8">
                <label class="form-label">Bidang Studi Terakhir <span class="text-danger">*</span></label>
                <input type="text" name="bidang_studi_terakhir" class="form-control" value="{{ old('bidang_studi_terakhir') }}" required>
              </div>
              <div class="col-12">
                <label class="form-label">Perguruan Tinggi Asal <span class="text-danger">*</span></label>
                <input type="text" name="perguruan_tinggi_asal" class="form-control" value="{{ old('perguruan_tinggi_asal') }}" required>
              </div>
              ...
              <div class="col-md-6">
                ...
                <input type="date" name="tmt_jabatan_fungsional" class="form-control" value="{{ old('tmt_jabatan_fungsional') }}">
              </div>
            </div>
          </div>
        </div>
              ...
              <div class="col-md-6">
                <div class="row g-3">
                  <div class="col-12">
                    ...
                    <input type="email" name="email_institusi" class="form-control" value="{{ old('email_institusi') }}" required placeholder="Contoh: dosen@kampus.ac.id">
                    <div class="form-text">Digunakan sebagai username login sistem.</div>
                  </div>
                  <div class="col-md-6">
                    <label class="form-label">Email Pribadi</label>
                    <input type="email" name="email_pribadi" class="form-control" value="{{ old('email_pribadi') }}">
                  </div>
                  <div class="col-md-6">
                ...
                <div class="row g-3 ps-md-3">
                  <div class="col-12">
                    <div class="form-check form-switch mt-2">
                      <input class="form-check-input" type="checkbox" role="switch" id="is_sertifikasi" name="is_sertifikasi_dosen" value="1" {{ old('is_sertifikasi_dosen') ? 'checked' : '' }}>
                  ...
                  <div class="col-12 serdos-field" style="display: none;">
                    ...
                    <input type="text" name="no_sertifikasi_dosen" class="form-control" value="{{ old('no_sertifikasi_dosen') }}" id="no_serdos">
                  </div>
                  <div class="col-md-6 serdos-field" style="display: none;">
                    ...
                    <input type="number" name="tahun_sertifikasi" class="form-control" value="{{ old('tahun_sertifikasi') }}" min="1900" id="th_serdos">
                  </div>
                  <div class="col-md-6 serdos-field" style="display: none;">
                    <label class="form-label">Bidang Sertifikasi <span class="text-danger">*</span></label>
                    <input type="text" name="bidang_sertifikasi" class="form-control" value="{{ old('bidang_sertifikasi') }}" id="bid_serdos">
                  </div>
                </div>
              </div>
            </div>
    ...
    <div class="d-flex justify-content-end gap-2 mt-4 mb-5">
      <a href="{{ route('dosen.index') }}" class="btn btn-light border px-4">Batal</a>
      <button type="submit" class="btn btn-primary px-5 shadow"><i class="ti ti-device-floppy me-2"></i>Simpan Data</button>
    </div>
  </form>
</main>
