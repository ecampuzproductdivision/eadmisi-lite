...
<main class="p-2">
  <div class="card border-1 mb-6">
    <div class="card-body p-4">
      <div class="row align-items-center">
        <div class="col">
          <h3 class="mb-1 fw-bold">Tambah Mahasiswa Baru</h3>
  ...
  <form action="{{ route('mahasiswa.store') }}" method="POST">
      ...
      <div class="card-body p-4">
        <div class="row g-3">
          ...
          <div class="col-md-3">
            ...
            <input type="text" name="no_passport" class="form-control @error('no_passport') is-invalid @enderror" value="{{ old('no_passport') }}">
      ...
      <div class="card-body p-4">
        <div class="row g-3">
          <div class="col-md-4">
            ...
            <select name="id_prodi" class="form-select @error('id_prodi') is-invalid @enderror" required>
              ...
              @foreach($prodiList as $p)
                <option value="{{ $p->prodiKode }}" {{ old('id_prodi') == $p->prodiKode ? 'selected' : '' }}>{{ $p->prodiNamaResmi }}</option>
              @endforeach
            </select>
          ...
          <div class="col-md-4">
            ...
            <select name="id_kurikulum" class="form-select @error('id_kurikulum') is-invalid @enderror" required>
              <option value="">-- Pilih Kurikulum --</option>
              @foreach($kurikulumList as $k)
                <option value="{{ $k->kurKode }}" {{ old('id_kurikulum') == $k->kurKode ? 'selected' : '' }}>{{ $k->kurNama }}</option>
              @endforeach
            </select>
            @error('id_kurikulum') <div class="invalid-feedback">{{ $message }}</div> @enderror
          </div>
          <div class="col-md-4">
            ...
            <select name="id_dosen_pa" class="form-select">
              ...
              @foreach($dosenList as $d)
                <option value="{{ $d->id_dosen }}" {{ old('id_dosen_pa') == $d->id_dosen ? 'selected' : '' }}>{{ $d->nama_lengkap }}</option>
              @endforeach
            </select>
          </div>
          <div class="col-md-2">
          ...
          <div class="col-md-2">
            <label class="form-label">Semester Masuk <span class="text-danger">*</span></label>
            <select name="semester_masuk" class="form-select" required>
              <option value="Ganjil" {{ old('semester_masuk') == 'Ganjil' ? 'selected' : '' }}>Ganjil</option>
              <option value="Genap" {{ old('semester_masuk') == 'Genap' ? 'selected' : '' }}>Genap</option>
            </select>
          </div>
          <div class="col-md-2">
          ...
          <div class="col-md-3">
            ...
            <select name="jalur_masuk" class="form-select" required>
              @foreach(['SNBP', 'SNBT', 'Mandiri', 'Pindahan', 'Alih Jenjang', 'Beasiswa'] as $j)
                <option value="{{ $j }}" {{ old('jalur_masuk') == $j ? 'selected' : '' }}>{{ $j }}</option>
              @endforeach
            </select>
          </div>
          <div class="col-md-3">
            <label class="form-label">Jenis Mahasiswa <span class="text-danger">*</span></label>
            <select name="jenis_mahasiswa" class="form-select" required>
              @foreach(['Reguler', 'Karyawan', 'Internasional', 'Pindahan', 'Transfer'] as $jm)
                <option value="{{ $jm }}" {{ old('jenis_mahasiswa') == $jm ? 'selected' : '' }}>{{ $jm }}</option>
              @endforeach
            </select>
          </div>
          <div class="col-md-3">
            ...
            <select name="status_mahasiswa" class="form-select" required>
              @foreach(['Aktif', 'Cuti', 'Tugas Belajar', 'Non-aktif'] as $sm)
                <option value="{{ $sm }}" {{ old('status_mahasiswa', 'Aktif') == $sm ? 'selected' : '' }}>{{ $sm }}</option>
              @endforeach
            </select>
          </div>
        </div>
      ...
      <div class="card-body p-4">
        <div class="row g-3">
          <div class="col-md-4">
            ...
            <input type="email" name="email_institusi" class="form-control @error('email_institusi') is-invalid @enderror" value="{{ old('email_institusi') }}" required>
          ...
          <div class="col-md-4">
            ...
            <input type="text" name="kota_asal" class="form-control" value="{{ old('kota_asal') }}" required>
          </div>
          <div class="col-md-4">
            <label class="form-label">Provinsi Asal <span class="text-danger">*</span></label>
            <input type="text" name="provinsi_asal" class="form-control" value="{{ old('provinsi_asal') }}" required>
          </div>
          <div class="col-md-4">
            <label class="form-label">Kode Pos</label>
            <input type="text" name="kode_pos_asal" class="form-control" value="{{ old('kode_pos_asal') }}">
          </div>
          <div class="col-md-8">
          ...
          <div class="col-md-4">
            ...
            <input type="text" name="kota_domisili" class="form-control" value="{{ old('kota_domisili') }}">
          </div>
        </div>
      </div>
    </div>
          ...
          <div class="col-md-3">
            ...
            <select name="penghasilan_ortu" class="form-select">
              ...
              @foreach(['< 1 juta', '1-3 juta', '3-6 juta', '6-10 juta', '> 10 juta'] as $ph)
                <option value="{{ $ph }}" {{ old('penghasilan_ortu') == $ph ? 'selected' : '' }}>{{ $ph }}</option>
              @endforeach
            </select>
          </div>
          <div class="col-md-3">
    ...
    <div class="d-flex justify-content-end gap-2 mb-4">
      <a href="{{ route('mahasiswa.index') }}" class="btn btn-light border px-4">Batal</a>
