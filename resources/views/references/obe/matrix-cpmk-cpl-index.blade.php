      ...
      <div class="d-flex justify-content-between align-items-center mb-2">
        <div>
          <h3 class="mb-1 fw-bold text-dark d-flex align-items-center gap-2">
            <i class="ti ti-layout-align-middle text-primary fs-3"></i>
            Matriks CPMK–CPL
          ...
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
              ...
              <li class="breadcrumb-item active">Matriks CPMK–CPL</li>
            </ol>
          </nav>
        </div>
      </div>
  ...
  <div class="card border-1 mb-4 shadow-sm" style="border-radius: 12px; background: linear-gradient(135deg, #f5f0ff 0%, #ede8f5 100%); border-left: 4px solid #6f42c1 !important;">
    <div class="card-body p-4">
      <div class="d-flex align-items-start gap-3">
        ...
        <div>
          <h6 class="fw-bold text-dark mb-1">Pemetaan Matriks Capaian Pembelajaran Mata Kuliah (CPMK) ke CPL (Matriks Mikro)</h6>
          <p class="text-muted small mb-0">
            Matriks CPMK-CPL menghubungkan CPMK spesifik per mata kuliah dengan CPL Program Studi yang didukungnya. Pilih kurikulum di bawah ini untuk mengelola matriks, mengatur bobot kontribusi (%), dan memvalidasi keselarasan dengan Matriks Makro CPL-MK.
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="row g-3">
      @foreach($kurikulums as $kur)
        @php
          ...
          $totalCpmk = $stat['total_cpmk'] ?? 0;
          $coveragePct = $stat['coverage_pct'] ?? 0;
            ...
            <div class="card-body p-4">
              ...
              <div class="row g-2 my-3">
                <div class="col-4 text-center border-end">
                  <div class="text-muted small fw-semibold" style="font-size: 10px;">Total CPL</div>
                  <div class="h5 mb-0 fw-bold text-dark">{{ $totalCpl }}</div>
                </div>
                <div class="col-4 text-center border-end">
                  <div class="text-muted small fw-semibold" style="font-size: 10px;">Total CPMK</div>
                  <div class="h5 mb-0 fw-bold text-dark">{{ $totalCpmk }}</div>
                </div>
                <div class="col-4 text-center">
                  <div class="text-muted small fw-semibold" style="font-size: 10px;">Isu Konsistensi</div>
                  <div class="h5 mb-0 fw-bold text-{{ $unmappedCplMk > 0 ? 'danger' : 'success' }}">{{ $unmappedCplMk }}</div>
                </div>
              </div>
              ...
              <div class="mb-3">
                <div class="d-flex justify-content-between small text-muted mb-1">
                  <span>Coverage CPL (Terdukung CPMK)</span>
                  <span class="fw-semibold">{{ $coveragePct }}%</span>
                </div>
                <div class="progress" style="height: 6px; border-radius: 4px;">
                  <div class="progress-bar {{ $coveragePct == 100 ? 'bg-success' : ($coveragePct >= 75 ? 'bg-primary' : 'bg-warning') }}"
                       style="width: {{ $coveragePct }}%; border-radius: 4px;"></div>
                </div>
              </div>
