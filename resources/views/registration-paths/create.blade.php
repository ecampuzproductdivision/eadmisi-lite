                    ...
                    <div class="col-md-3 col-12">
                        ...
                        <select name="periode_id" id="periode_id" class="form-select @error('periode_id') is-invalid @enderror" required>
                            ...
                            @foreach($periodes as $periode)
                                <option value="{{ $periode->id }}" {{ old('periode_id', $activePeriodeId ?? '') == $periode->id ? 'selected' : '' }}>
                                    {{ $periode->label }} {!! $periode->status_aktif ? '<span class="badge bg-success-subtle text-success ms-1">Aktif</span>' : '' !!}
                                </option>
                            @endforeach
                        </select>
